...
async function runTests() {
  ...
  try {
    ...
    assert.ok(generatedPrompt.includes('USER REQUEST: "redo changes"'), 'prompt 应包含用户请求');
    assert.ok(generatedPrompt.includes('--- HISTORY CONTEXT ---'), 'prompt 应包含历史上下文块');
    assert.ok(generatedPrompt.includes('mock 历史提示'), 'prompt 应包含历史 prompt');
    assert.ok(generatedPrompt.includes('Operations:'), 'prompt 应包含 operations');
    assert.ok(generatedPrompt.includes('"type": "create"'), 'prompt 应包含 operations 细节');
    console.log('  ✓ 通过\n');
